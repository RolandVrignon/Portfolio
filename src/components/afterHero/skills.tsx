...
export const SkillSection = () => {
  ...
  const text = `Hi, I am Roland Vrignon,\n
                a passionate developer\n
                and a design enthusiast`;

  // On sépare par paragraphes
  const paragraphs = text.split("\n").filter((line) => line.trim() !== "");
  const wordsPerParagraph = paragraphs.map((p) => p.trim().split(" ")); // Nettoyage des espaces
  ...
  useMotionValueEvent(
    useScroll({
      target: containerRef,
      offset: ["0.2 start", "end end"], // Décalage de 20% avant de commencer le reveal
    }).scrollYProgress,
    "change",
    (latest) => {
      if (isInView) {
  ...
  return (
    <section id="skills-section" ref={containerRef}>
      ...
      <div className="w-screen h-[200vh] bg-white">
        <div className="sticky top-0 flex flex-col items-start justify-end h-[100vh] w-screen p-10">
          {/* Texte aligné à gauche */}
          <div className="whitespace-pre-wrap text-[11.5vw] leading-[10vw] text-left ml-0">
